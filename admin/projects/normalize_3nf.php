...
// 3. Logları Çek (3NF Aşaması İçin)
$stmtLogs = $db->prepare("SELECT * FROM project_normalization_logs WHERE project_id = ? AND stage = '3NF' ORDER BY id DESC");
$stmtLogs->execute([$project_id]);
$logs = $stmtLogs->fetchAll(PDO::FETCH_ASSOC);

// 4. DURUM KONTROLÜ (Log varsa bitmiştir)
...
?>
    ...
    <div class="mb-8 border-b border-slate-200 pb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-900">Stage 5: Normalization (3NF - Final)</h1>
            ...
            <p class="text-slate-500 text-sm mt-1">Eliminate transitive dependencies (non-key columns depending on other non-key columns).</p>
        </div>

        <div class="flex gap-2">
            <a href="normalize_2nf?id=<?php echo $project_id; ?>" class="btn-sm bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 flex items-center transition shadow-sm">
                <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i> Previous: 2NF
            </a>

            <?php if($isCompleted): ?>
                <a href="er_diagram?id=<?php echo $project_id; ?>" class="btn-sm bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center shadow-lg transform hover:-translate-y-0.5 transition">
                    Stage 6: ER Diagram <i data-lucide="arrow-right" class="w-4 h-4 ml-2"></i>
                </a>
            <?php else: ?>
                <button disabled class="btn-sm bg-slate-100 text-slate-400 px-5 py-2 rounded-lg text-sm font-medium cursor-not-allowed flex items-center">
                    Next: ER Diagram <i data-lucide="arrow-right" class="w-4 h-4 ml-2"></i>
                </button>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-6 shadow-sm">
                <h3 class="text-indigo-900 font-bold mb-2 flex items-center">
                    <i data-lucide="layers" class="w-5 h-5 mr-2"></i> 3NF Transformation
                </h3>
                <p class="text-sm text-indigo-800/80 mb-6 leading-relaxed">
                    The AI will scan for <strong>Transitive Dependencies</strong>. If Column A determines Column B, and Column B determines Column C, then C should be moved to a new table.
                </p>

                <button onclick="apply3NF()" id="applyBtn" class="w-full py-3 <?php echo $isCompleted ? 'bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50' : 'bg-indigo-600 hover:bg-indigo-700 text-white'; ?> rounded-lg font-bold shadow-sm transition flex justify-center items-center">
                    <?php if($isCompleted): ?>
                        <i data-lucide="refresh-cw" class="w-4 h-4 mr-2"></i> Re-Apply 3NF
                    <?php else: ?>
                        <i data-lucide="play" class="w-4 h-4 mr-2"></i> Apply 3NF Logic
                    <?php endif; ?>
                </button>
            </div>
        ...
        <div class="lg:col-span-8">
            ...
            <?php if(empty($tables)): ?>
                <div class="bg-slate-50 border border-slate-200 rounded-xl p-16 text-center border-dashed">
                    ...
                    <p class="text-slate-400 text-sm mt-1">Please ensure you completed previous steps.</p>
                </div>
            <?php else: ?>
    ...
    <script>
        function apply3NF() {
            // Re-apply uyarısı
            <?php if($isCompleted): ?>
            if(!confirm('Attention: Re-applying 3NF logic will update the schema. Continue?')) return;
            <?php else: ?>
            if(!confirm('AI will analyze the schema and enforce 3NF rules (Transitive Dependencies). Continue?')) return;
            <?php endif; ?>
            ...
            fetch('/services/ai/api/normalize_3nf.php', {
                method: 'POST',
                body: formData
            })
                .then(async res => {
