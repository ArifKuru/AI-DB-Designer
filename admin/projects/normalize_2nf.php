...
// 3. Logları Çek (2NF Aşaması İçin)
$stmtLogs = $db->prepare("SELECT * FROM project_normalization_logs WHERE project_id = ? AND stage = '2NF' ORDER BY id DESC");
$stmtLogs->execute([$project_id]);
$logs = $stmtLogs->fetchAll(PDO::FETCH_ASSOC);

// 4. DURUM KONTROLÜ (Mantık: Log varsa bitmiştir)
...
?>
    ...
    <div class="mb-8 border-b border-slate-200 pb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-900">Stage 5: Normalization (2NF)</h1>
            ...
            <p class="text-slate-500 text-sm mt-1">Eliminate partial dependencies in composite keys.</p>
        </div>

        <div class="flex gap-2">
            <a href="normalize_1nf?id=<?php echo $project_id; ?>" class="btn-sm bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 flex items-center transition shadow-sm">
                <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i> Previous: 1NF
            </a>

            <?php if($isCompleted): ?>
                <a href="normalize_3nf?id=<?php echo $project_id; ?>" class="btn-sm bg-slate-900 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-slate-800 flex items-center shadow-lg transform hover:-translate-y-0.5 transition">
                    Next: 3NF <i data-lucide="arrow-right" class="w-4 h-4 ml-2"></i>
                </a>
            <?php else: ?>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-6 shadow-sm">
                <h3 class="text-indigo-900 font-bold mb-2 flex items-center">
                    <i data-lucide="layers" class="w-5 h-5 mr-2"></i> 2NF Transformation
                </h3>
                <p class="text-sm text-indigo-800/80 mb-6 leading-relaxed">
                    The AI will scan for Composite Primary Keys. If a column depends on only <em>part</em> of the key, it will be moved to a separate table.
                </p>

                <button onclick="apply2NF()" id="applyBtn" class="w-full py-3 <?php echo $isCompleted ? 'bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50' : 'bg-indigo-600 hover:bg-indigo-700 text-white'; ?> rounded-lg font-bold shadow-sm transition flex justify-center items-center">
                    <?php if($isCompleted): ?>
                        <i data-lucide="refresh-cw" class="w-4 h-4 mr-2"></i> Re-Apply 2NF
                    <?php else: ?>
                        <i data-lucide="play" class="w-4 h-4 mr-2"></i> Apply 2NF Logic
                    <?php endif; ?>
                </button>
            </div>
    ...
    <script>
        function apply2NF() {
            // Re-apply uyarısı
            <?php if($isCompleted): ?>
            if(!confirm('Attention: Re-applying 2NF logic might change the table structure (Partial Dependencies). Continue?')) return;
            <?php else: ?>
            if(!confirm('AI will analyze the schema and enforce 2NF rules (Partial Dependencies). Continue?')) return;
            <?php endif; ?>
            ...
            fetch('/services/ai/api/normalize_2nf.php', {
                method: 'POST',
                body: formData
            })
                .then(async res => {
